      ...
      <section className="chat-main">
        ...
        <div className="chat-toolbar">
          <div className="chat-toolbar-selects">
            <label title="Select the model server used for chat requests">{text.runner}<select title="Select the model server used for chat requests" value={runnerId} onChange={e => setRunnerId(e.target.value)} disabled={loadingModels && runners.length < 1}>{loadingModels && runners.length < 1 ? <option value="">Loading...</option> : runners.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
            <label title="Select any model returned by the configured server">{text.model}<select title="Select the model to use for chat requests" value={model} onChange={e => setModel(e.target.value)} disabled={modelOptions.length < 1}>{modelOptions.length < 1 ? <option value="">{loadingModels ? 'Loading...' : text.noModels}</option> : modelOptions.map(item => <option key={item}>{item}</option>)}</select></label>
